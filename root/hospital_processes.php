 ...
 * @Last Modified time: 2018-11-22 15:51:56
 */
								...
								function downloadFilea( $type, $key   ) {
									...
									function getNotification( $type , $from, $limit  ) {
										...
										try {
											...
											$doc_hospital = selectFromTable (' hos_district  ', '  doc_hospital ' , ' use_id = '.decrypt($_SESSION[SYSTEM_NAME.'userid0']).'  '     , $a ); 	
												...
												foreach ($result as $key => $value) {
													...
													try {
														...
														foreach ($arrady as $key => $valueIn) {
															if($doc_hospital) {  
																if(strtolower(trim($valueIn)) == strtolower(trim($doc_hospital))  ) {
